...
rl.on('line', function (line) {
    lines.push(line);
    if(Number(lines[0]) === lines.length - 2){
        console.log(getResult(lines[1],lines.slice(2)));
        lines.length = 0;
    }
});
 ...
 * 
 * @param {*} str string
 * @param {*} instructionLines string[]
 */
function getResult(str,instructionLines){
    let pos = 0;
    for(const line of instructionLines){
        ...
        switch(instruction){
            ...
            case 'SEARCH-BACKWARD':
                tmp = str.indexOf(param);
                if(tmp !== -1){
                    pos = tmp + param.length;
                }
                break;
            case 'INSERT':
                str = str.slice(0,pos) + param + str.slice(pos);
                pos += param.length;
                break;
            case 'REPLACE':
                str = str.slice(0,pos) + param;
                break;
            case 'DELETE':
                tmp = Number(param);
                str = str.slice(0,pos) + str.slice(pos + tmp);
                break;
        }
    }
    return str;
